...
const handlePredict = async () => {
  ...
  if (/^[0-9\s]+$/.test(trimmedReview)) {
    return showAlert(
      "Invalid Review",
      "Reviews cannot contain only numbers. Please write about food 🍔🍕🥗.",
      "warning"
    );
  }
  ...
  try {
    ...
    // 🚫 AI check for food-related content
    if (data.is_food_related === false) {
      return showAlert(
        "Not Food Related",
        "Please enter a review related to food 🍔🍕🥗.",
        "warning"
      );
    }
